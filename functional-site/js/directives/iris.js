    ...
    .directive('processlist', function($rootScope) {
        return {
            link: function(scope, element, attrs, routeParams) {
                //$loginbox = kbaseLogin($('#login'));                

                $rootScope.plist = kbaseIrisProcessList($(element));

                $rootScope.term = kbaseIrisTerminal($.jqElem('div'), {
                    'promptIfUnauthenticated'   : 0,
                    'invocationURL'             : 'http://kbase.us/services/invocation',
                    'commandsElement'           : $rootScope.commandlist,
kbaseIrisGrammar(                    'grammar'                   : $('<div></div>')),
...
kbaseIrisCommands(                $(element), {
        ...
        return {
           link: function(scope, element, attrs, routeParams) {
                var $fb = kbaseIrisFileBrowser($(element), {
                        client : $rootScope.term.client(),
                        ...
                        editFileCallback : function (file, $fb) {
                            ...
                            var $editor = kbaseIrisFileEditor($.jqElem('div'), {
                                    file : file,
                                    client : $fb.client(),
