...
$.KBWidget({
    ...
    function manageWorkspaceSave(workspace, modal) {
        ...
        // first check if any username inputs are empty
        var error = false;
        newPerms.find('input').each(function(i, el) {
            // clear 'control-group error' (from before)
            $(el).parent().removeClass('control-group error');

        // new user permissions cannot be empty
            if ($(el).val() === '') {
        ...
        // check if global perm has changed
        ...
        // check if user perms has changed
        var userPerms = {};

        // look through existing permissions
    ...
    // show the clone workspace modal
    function cloneWorkspace(workspace, manageModal) {
        var modal = new Modal();

        modal.setTitle('Clone Workspace');
        modal.setContent(
            '<div><h4 style="margin-top: 0px;">' + workspace.id + '</h4>'
                + '<table style="margin-left: auto; margin-right: auto; text-align: right; width: 300px;">'
                + '<tr style="height: 42px;"><td style="width: 100%;">Clone into:</td>'
                + '<td style="padding-right: 30px;">'
                + '<label class="radio inline" style="padding-right: 5px; padding-top: 0px;">'
                + '<input id="new-input" type="radio" name="clone-into" value="new" checked />new'
                + '</label><label class="radio inline" style="padding-right: 5px; padding-top: 0px;">'
                + '<input id="existing-input" type="radio" name="clone-into" value="existing" />existing'
                + '</label></td></tr>'
                + '<tr><td>Workspace:</td>'
                + '<td id="clone-into-cell"></td></tr>'
                + '<tr id="clone-permission-row"><td>Global Permission:</td>'
                + '<td>' + createPermissionSelect('clone-permission', workspace.global_permission)
                + '</td></tr></table></div>'
        );
