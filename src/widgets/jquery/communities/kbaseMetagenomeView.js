 ...
 */
define(['jquery', 'kb.jquery.authenticatedwidget', 'kb.service.workspace', 'kb.app', 'kb.html', 'kb.jquery.tabs', 'datatables' ],
    function ($, _AuthenticatedWidget, Workspace, App, H) {
        'use strict';
        $.KBWidget({
            name: 'MetagenomeView',
            ...
            render: function () {
                ...
                kbws.get_objects([{ref: self.options.ws + "/" + self.options.id}], function (data) {
                    ...
                    if (data.length === 0) {
                        ...
                        $('#' + pref + 'metadata').html(H.makeTable(columns, mdata));
