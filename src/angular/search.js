define(['kb.runtime', 'kb.session', 'postal', 'angular', 'kb.client.narrativemanager', 'angular-ui-router', 'angular-ui-bootstrap', 'angular-ui-bootstrap-templates', 'jquery.blockUI'],
    function (R, Session, postal, angular, NarrativeManager, ui_router, ui_bootstrap) {
        ...
        searchApp.config(function ($httpProvider, $stateProvider, $provide, $compileProvider) {
        ...
        searchApp.controller('searchController', function searchCtrl($rootScope, $scope, $q, $timeout, $http, $state, $stateParams, searchCategoryLoadService, searchOptionsService, searchKBaseClientsService, require) {
            ...
            console.log(searchOptionsService);
            ...
            $scope.startSearch = function () {
                ...
                var init = function () {
                    ...
                    console.log(searchCategoryLoadService);
                ...
                var captureState = function () {
                    ...
                    if ($stateParams.facets) {
                        ...
                        console.log($scope.options);
                        if ($scope.options.searchOptions.perCategory[$scope.options.selectedCategory]) {
            ...
            $scope.listWorkspaces = function () {
                try {
                    ...
                    $scope.workspace_service = searchKBaseClientsService.getWorkspaceClient();
        ...
        function startSearchApp(node) {
            angular.bootstrap(node, ['search']);
        }
        function stopSearchApp() {

        }
        function search(node, path, params) {
            var n = angular.element(node).find('.kbase-search-view');
            console.log(n);
            var c = n.scope();
            console.log('scope?');
            console.log(c);
            console.log(c.newSearch);
            //c.go('search', params, false);
            //    c.newSearch();
        }

        return {
            start: startSearchApp,
            stop: stopSearchApp,
            search: search
        }
    });
