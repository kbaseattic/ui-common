...
return KBWidget=function(name,parent,def){if(widgetRegistry[name]!=undefined){return;}
var Widget=function($elem){this.$elem=$elem;this.options=$.extend(true,{},def.options);return this;}
...
return $.fn[name];}
return KBWidget('kbaseWidget',{options:{},element:function(){return this;},dbg:function(txt){if(window.console)console.log(txt);},init:function(args){var opts=$.extend(true,{},this.options);this.options=$.extend(false,{},opts,args);return this;},alert:function(msg){if(msg==undefined){msg=this.data('msg');}
...
else{return 0}},trigger:function(){this.$elem.trigger.apply(this.$elem,arguments);},on:function(){this.$elem.on.apply(this.$elem,arguments);},});}(jQuery));(function($,undefined){return KBWidget("kbaseAuthenticatedWidget",'kbaseWidget',{version:"1.0.0",options:{},init:function(options){this._super(options);if(options.$loginbox){this.$loginbox=options.$loginbox;}
else{throw"Cannot create authenticated widget w/o login box!";}
return this;},sessionId:function(){return this.$loginbox.sessionId();},});}(jQuery));(function($,undefined){return KBWidget("kbaseAccordion",'kbaseWidget',{version:"1.0.0",options:{fontSize:'100%',},init:function(options){this._super(options);if(this.options.client){this.client=this.options.client;}
...
.css('width','100%')
.bind('click',function(e){e.preventDefault();e.stopPropagation();if(canCollapse){$(this).parent().parent().children().last().collapse('toggle');}})
.append($('<span></span>')
.attr('id','title'))
.append($('<div></div>')
.addClass('btn-group')
...
.css('font-size','70%')
.css('color','gray')
.append(this.options.postcontent));this._rewireIds($div,this);this.setControls(this.options.controls);this.setTitle(this.options.title);this.setContent(this.options.content);$elem.append($div);return this;},setTitle:function(title){this.data('title').empty();this.data('title').append(title);},setContent:function(content){this.data('content').empty();this.data('content').append(content);},controls:function(control){if(control){return this._controls[control];}
...
.bind('click',function(e){e.preventDefault();e.stopPropagation();val.callback.call(this,e,$box);});if(val.id){this._controls[val.id]=$button;}
...
chips.success=1;return field==undefined?chips:chips[field];},sessionId:function(){return this.get_kbase_cookie('kbase_session_id');},token:function(){return this.get_kbase_cookie('token');},init:function(options){this._super(options);var kbaseCookie=this.get_kbase_cookie();this.$elem.empty();var style='_'+this.options.style+'Style';this.ui=this[style]();if(this.ui){this.$elem.append(this.ui);}
...
.attr('href','#')
.text('Sign In')
.bind('click',$.proxy(function(e){this.openDialog();},this)))
.append($('<div></div>')
.attr('id','userdisplay')
.css('display','none')
.append($('<div></div>')
...
.bind('click',$.proxy(function(e){this.logout();},this))
.append($('<i></i>').addClass('icon-signout'))));this._rewireIds($prompt,this);this.registerLogin=function(args){if(args.success){this.data("loginlink").hide();this.data('loggedinuser_id').text(args.name);this.data("userdisplay").show();this.data('loginDialog').closePrompt();}
...
if(value!=kbaseDeletePrompt(undefined){$tr.append($td);}},this));return $tr;},deletePrompt:function(row){var $deleteModal=$('<div></div>'), {name:row,callback:this.deleteRowCallback(row),});$deleteModal.openPrompt();},deleteRowCallback:function(row){},shouldDeleteRow:function(row){return 1;},});}(jQuery));(function($,undefined){return KBWidget("kbaseTabs",'kbaseWidget',{version:"1.0.0",options:{tabPosition:'top',canDelete:false,borderColor:'lightgray',},init:function(options){this._super(options);this.data('tabs',{});this.data('nav',{});this.appendUI($(this.$elem));return this;},appendUI:function($elem,tabs){if(tabs==undefined){tabs=this.options.tabs;}
var $block=$('<div></div>')
...
if(tab.show||tabCount=kbaseDeletePrompt(=1){this.showTab(tab.tab);}},closeIcon:function(){return'icon-remove';},deleteTabToolTip:function(tabName){return'Remove '+tabName;},showTab:function(tab){if(this.shouldShowTab(tab)){this.data('nav')[tab].find('a').trigger('click');}},shouldShowTab:function(tab){return 1;},deletePrompt:function(tabName){var $deleteModal=$('<div></div>'), {name:tabName,callback:this.deleteTabCallback(tabName),});$deleteModal.openPrompt();},deleteTabCallback:function(tabName){return $.proxy(function(e,$prompt){if($prompt!=undefined){$prompt.closePrompt();}
...
if(this.shouldDeleteTab(tabName)){$tab.remove();$nav.remove();}},this);},shouldDeleteTab:function(tabName){return 1;},activeTab:function(){var activeNav=this.data('tabs-nav').find('.active:last a')[0];return $(activeNav).attr('data-tab');},});}(jQuery));(function($,undefined){return KBWidget("kbaseFileBrowser",'kbaseWidget',{version:"1.0.0",options:{'root':'/','controls':true,'externalControls':true,'height':'110px','tallHeight':'450px','shouldToggleNavHeight':true,'controlButtons':['deleteButton','viewButton','addDirectoryButton','uploadButton','addButton'],'name':'File Browser','openFolderIcon':'icon-folder-open-alt','closedFolderIcon':'icon-folder-close-alt','fileIcon':'icon-file',},init:function(options){this._super(options);if(options.client){this.client=options.client;}
if(options.$loginbox){this.$loginbox=options.$loginbox;}
this.appendUI(this.$elem);return this;},refreshDirectory:function(path){if(this.sessionId()==undefined){this.$elem.find('ul').first().empty();}
if(this.data(path)){this.listDirectory(path,this.data(path));}},sortByName:function(a,b){if(a['name'].toLowerCase()<b['name'].toLowerCase()){return-1}
else if(a['name'].toLowerCase()>b['name'].toLowerCase()){return 1}
else{return 0}},sessionId:function(){return this.$loginbox.sessionId();},selected:function(path){},displayPath:function(path,$ul,filelist){var $fb=this;$ul.empty();$.each(filelist,$.proxy(function(idx,val){var icon=this.options.fileIcon;var callback=function(e){e.preventDefault();$fb.data('activeDirectory',undefined);$fb.data('activeFile',undefined);$fb.disableButtons();var $opened=$fb.$elem.find('.active');$opened.removeClass('active');if($(this).parent().get(0)!=$opened.get(0)){$(this).parent().addClass('active');$fb.data('activeFile',val.path);$fb.enableButtons('f');$fb.selected(val.path);}};if(val.type=='directory'){icon=this.data(val.path)?$fb.options.openFolderIcon:$fb.options.closedFolderIcon
callback=function(e){e.preventDefault();$fb.data('activeDirectory',undefined);$fb.data('activeFile',undefined);$fb.disableButtons();var $opened=$fb.$elem.find('.active');$opened.removeClass('active');if($(this).next().children().length){if($(this).parent().get(0)==$opened.get(0)){$(this).children().first().removeClass($fb.options.openFolderIcon);$(this).children().first().addClass($fb.options.closedFolderIcon);$(this).next().empty();$fb.data(val.path,undefined);}
else{$fb.data('activeDirectory',val.path);$(this).parent().addClass('active');$fb.enableButtons('d');$fb.selected(val.path);}}
else{$(this).children().first().removeClass($fb.options.closedFolderIcon);$(this).children().first().addClass($fb.options.openFolderIcon);$fb.listDirectory(val.path,$(this).next());$(this).parent().addClass('active');$fb.data('activeDirectory',val.path);$fb.data(val.path,$(this).next());$fb.enableButtons('d');$fb.selected(val.path);}}}
$ul.append($('<li></li>')
.append($('<a></a>')
.append($('<i></i>')
.addClass(icon)
...
.data('meta',val.meta)
.data('able','baker')
.append($('<ul></ul>').addClass('nav nav-list')));if(val.type=='directory'&&this.data(val.path)){this.listDirectory(val.path,$ul.children().last().children().last());}},this));},fileBrowserContainer:function(){var navHeight=this.options.height;var $ul=$('<ul></ul>')
.addClass('nav nav-list')
.css('height',navHeight)
.css('overflow','auto')
.attr('id','ul-nav');var $container=$('<div></div>')
.append($ul);this.data('file-ul',$ul);return $container;},fileBrowserControls:function(){var $div=$('<div></div>')
...
.attr('id','fileInput')
.css('display','none')
.bind('change',jQuery.proxy(this.handleFileSelect,this))));return $div;},appendUI:function($elem){var $container=kbaseBox(this.fileBrowserContainer();var $box=$('<div></div>'), {title:this.options.name,canCollapse:true,content:$container,});$elem.append($box.$elem);if(this.options.controls){if(this.options.externalControls){$container.parent().parent().append(this.fileBrowserControls());}
else{$container.append(this.fileBrowserControls());}}
this._rewireIds($box.$elem,this);$.each(this.options.controlButtons,$.proxy(function(idx,val){this.data('control-buttons').append(this[val]());},this));this._rewireIds($box.$elem,this);this.listDirectory(this.options.root,this.data('file-ul'));this.disableButtons();return this;},disableButtons:function(){this.toggleButtons('N');},enableButtons:function(flag){this.toggleButtons(flag);},toggleButtons:function(flag){$.each(this.options.controlButtons,$.proxy(function(idx,val){var $button=this[val]();if($button==undefined){return;}
var require=$button.data('require');if(require!=undefined){if((require=='a'&&flag!='N')||require==flag){$button.removeClass('disabled');}
else{$button.addClass('disabled');}}},this));},addDirectoryButton:function(){return this.data('addDirectoryButton')!=undefined?this.data('addDirectoryButton'):$('<a></a>')
.attr('id','addDirectoryButton')
.addClass('btn btn-mini')
.append($('<i></i>').addClass('icon-plus'))
.attr('title','Add new directory')
.tooltip()
.bind('click',$.proxy(function(e){e.preventDefault();if(!this.addDirectoryButton().hasClass('disabled')){this.addDirectory();}},this))},viewButton:function(){return this.data('viewButton')!=undefined?this.data('viewButton'):$('<a></a>')
.attr('id','viewButton')
.addClass('btn btn-mini')
.append($('<i></i>').addClass('icon-search'))
.attr('title','View selected file')
.tooltip()
.bind('click',$.proxy(function(e){e.preventDefault();if(!this.viewButton().hasClass('disabled')){this.openFile(this.data('activeFile'));}},this))
.data('require','f')},deleteButton:function(){return this.data('deleteButton')!=undefined?this.data('deleteButton'):$('<a></a>')
.attr('id','deleteButton')
.addClass('btn btn-mini')
.append($('<i></i>').addClass('icon-minus'))
.attr('title','Delete selected item')
.tooltip()
.bind('click',$.proxy(function(e){e.preventDefault();if(!this.deleteButton().hasClass('disabled')){this.deleteFile();}},this))
.data('require','a')},uploadButton:function(){return this.data('uploadButton')!=undefined?this.data('uploadButton'):$('<a></a>')
.attr('id','uploadButton')
.addClass('btn btn-mini')
.append($('<i></i>').addClass('icon-arrow-up'))
.attr('title','Upload file')
.tooltip()
.bind('click',$.proxy(function(e){this.data('fileInput').trigger('click');},this))},addButton:function(){if(this.options.addFileCallback==undefined){return;}
return this.data('addButton')!=undefined?this.data('addButton'):$('<a></a>')
.attr('id','addButton')
.addClass('btn btn-mini')
.append($('<i></i>').addClass('icon-arrow-right'))
.attr('title','Add file')
.tooltip()
.bind('click',$.proxy(function(e){e.preventDefault();e.stopPropagation();if(!this.addButton().hasClass('disabled')){this.options.addFileCallback(this.data('activeFile'));}},this))
.data('require','f')},addDirectory:function(){var parentDir=kbasePrompt(this.data('activeDirectory')||this.options.root;var that=this;var displayDir=parentDir.replace(this.options.root,'/');var $addDirectoryModal=$('<div></div>'), {title:'Create directory',body:$('<p></p>')
...
.attr('type','text')
.attr('name','dir_name')
.attr('size','20')),controls:['cancelButton',{name:'Create directory',type:'primary',callback:function(e,$prompt){$prompt.closePrompt();that.makeDirectoryCallback($addDirectoryModal.dialogModal().find('input').val(),parentDir);}}]});$addDirectoryModal.openPrompt();},openFile:function(file,content,win){if(win==undefined){win=window.open();win.document.open();}
if(content==undefined){this.fetchContent(file,win);return;}
content=content.replace(/>/g,'&gt;');content=content.replace(/</g,'&lt;');win.document.write($('<div></div>').append($('<div></div>')
.css('white-space','pre')
.append(content))
.html());win.document.close();},deleteFile:function(){var file=this.data('activeFile');var deleteMethod='deleteFileCallback';if(file==undefined){file=this.data('activeDirectory');if(file==undefined){return;}
file=file.replace(/\/+$/,'');deleteMethod='deleteDirectoryCallback';}
var matches=file.match(/(.+)\/[^/]+$/);var active_dir='/';if(matches!=undefined&&matches.length>1){active_dir=matches[1];}
var that=kbaseDeletePrompt(this;var promptFile=file.replace(this.options.root,'');var $deleteModal=$('<div></div>'), {name:promptFile,callback:function(e,$prompt){$prompt.closePrompt();that[deleteMethod](file,active_dir);}});$deleteModal.openPrompt();},handleFileSelect:function(evt){evt.stopPropagation();evt.preventDefault();var files=evt.target.files||evt.originalEvent.dataTransfer.files||evt.dataTransfer.files;$.each(files,jQuery.proxy(function(idx,file){var reader=new FileReader();var upload_dir=this.options.root;if(this.data('activeDirectory')){upload_dir=this.data('activeDirectory');}
var $processElem;if(this.options.processList){$processElem=this.options.processList.addProcess('Uploading '+file.name);}
reader.onload=jQuery.proxy(function(e){this.uploadFile(file.name,e.target.result,upload_dir,$processElem);},this);reader.readAsText(file);},this));this.data('fileInput').val('');},listDirectory:function(path,$ul){throw"Cannot call listDirectory directly - please subclass";},makeDirectoryCallback:function(dir,parentDir){throw"Cannot call makeDirectoryCallback directly - please subclass";},fetchContent:function(file,win){throw"Cannot call fetchContent directly - please subclass";},deleteFileCallback:function(file,active_dir){throw"Cannot call deleteFileCallback directly - please subclass";},deleteDirectoryCallback:function(file,active_dir){throw"Cannot call deleteDirectoryCallback directly - please subclass";},uploadFile:function(name,content,upload_dir,$processElem){throw"Cannot call uploadFile directly - please subclass";},});}(jQuery));(function($,undefined){return KBWidget("kbaseIrisCommands",'kbaseAccordion',{version:"1.0.0",options:{link:function(evt){alert("clicked on "+$(evt.target).text());},englishCommands:0,fontSize:'75%',overflow:true,sectionHeight:'300px',},init:function(options){this._super(options);if(options.client){this.client=options.client;}
this.commands=[];return this;},completeCommand:function(command){var completions=[];var commandRegex=new RegExp('^'+command+'.*');for(var idx=0;idx<this.commands.length;idx++){if(this.commands[idx].match(commandRegex)){completions.push(this.commands[idx]);}}
...
return matches.sort();},appendUI:function($elem){this.client.valid_commands_async($.proxy(function(res){var commands=[];$.each(res,$.proxy(function(idx,group){group.title;var $ul=$('<ul></ul>')
.addClass('unstyled')
...
this.commands.push(val.cmd);$ul.append(this.createLI(val.cmd,label));},this));commands.push({'title':group.title,'body':$ul});},this));this.loadedCallback($elem,commands);},this));},createLI:function(cmd,label,func){if(label==undefined){label=cmd;}
if(func==undefined){func=this.options.link;}
var $commands=this;return $('<li></li>')
.bind('mouseover',function(e){e.preventDefault();$(this).children().last().css('display','inline');})
.bind('mouseout',function(e){e.preventDefault();$(this).children().last().css('display','none');})
.append($('<a></a>')
.attr('href','#')
.attr('title',cmd)
...
.css('float','right')
.append('?')
.bind('click',function(e){e.preventDefault();if($commands.options.terminal!=undefined){$commands.options.terminal.run(cmd+' -h');}}))
...
.addClass('unstyled')
.css('max-height',this.options.overflow?this.options.sectionHeight:'5000px')
.css('overflow',this.options.overflow?'auto':'visible'));$elem.append($div);this._rewireIds($div,this);this._superMethod('appendUI',$div,commands);this.data('accordion').css('margin-bottom','0px');},});}(jQuery));(function($,undefined){return KBWidget("kbaseIrisFileBrowser",'kbaseWidget',{version:"1.0.0",options:{'root':'/','controls':true,'externalControls':true,'height':'110px','tallHeight':'450px','shouldToggleNavHeight':true,'controlButtons':['deleteButton','viewButton','addDirectoryButton','uploadButton'],'openFolderIcon':'icon-folder-open-alt','closedFolderIcon':'icon-folder-close-alt','fileIcon':'icon-file',},init:function(options){this._super(options);if(options.client){this.client=options.client;}
if(options.$loginbox){this.$loginbox=options.$loginbox;}
this.appendUI(this.$elem);return this;},refreshDirectory:function(path){if(this.sessionId()==undefined){this.$elem.find('ul').first().empty();}
if(this.data(path)){this.listDirectory(path,this.data(path));}},sortByName:function(a,b){if(a['name']<b['name']){return-1}
else if(a['name']>b['name']){return 1}
else{return 0}},sessionId:function(){return this.$loginbox.sessionId();},listDirectory:function(path,$ul){this.data(path,$ul);this.client.list_files_async(this.sessionId(),'/',path,jQuery.proxy(function(filelist){var dirs=filelist[0];var files=filelist[1];var $fb=this;$ul.empty();jQuery.each(dirs.sort(this.sortByName),$.proxy(function(idx,val){val['full_path']=val['full_path'].replace(/\/+/g,'/');$ul.append($('<li></li>')
.append($('<a></a>')
.append($('<i></i>')
...
.append(val['name'])
.attr('href','#')
.bind('click',function(e){e.preventDefault();$fb.data('activeDirectory',undefined);$fb.data('activeFile',undefined);$fb.viewButton().addClass('disabled');$fb.deleteButton().addClass('disabled');var $opened=$fb.$elem.find('.active');$opened.removeClass('active');if($(this).next().children().length){if($(this).parent().get(0)==$opened.get(0)){$(this).children().first().removeClass($fb.options.openFolderIcon);$(this).children().first().addClass($fb.options.closedFolderIcon);$(this).next().empty();$fb.data(val['full_path'],undefined);}
else{$fb.data('activeDirectory',val['full_path']);$(this).parent().addClass('active');$fb.deleteButton().removeClass('disabled');}}
else{$(this).children().first().removeClass($fb.options.closedFolderIcon);$(this).children().first().addClass($fb.options.openFolderIcon);$fb.listDirectory(val['full_path'],$(this).next());$(this).parent().addClass('active');$fb.data('activeDirectory',val['full_path']);$fb.deleteButton().removeClass('disabled');}}))
.append($('<ul></ul>').addClass('nav nav-list')));if(this.data(val['full_path'])){this.listDirectory(val['full_path'],$ul.children().last().children().last());}},this));jQuery.each(files.sort(this.sortByName),$.proxy(function(idx,val){val['full_path']=val['full_path'].replace(/\/+/g,'/');$ul.append($('<li></li>')
.append($('<a></a>')
.append($('<i></i>')
...
.append(val['name'])
.attr('href','#')
.bind('click',function(e){e.preventDefault();$fb.data('activeDirectory',undefined);$fb.data('activeFile',undefined);$fb.viewButton().addClass('disabled');$fb.deleteButton().addClass('disabled');var $opened=$fb.$elem.find('.active');$opened.removeClass('active');if($(this).parent().get(0)!=$opened.get(0)){$(this).parent().addClass('active');$fb.data('activeFile',val['full_path']);$fb.viewButton().removeClass('disabled');$fb.deleteButton().removeClass('disabled');}})));},this));},this),$.proxy(function(err){this.dbg(err)},this));},toggleNavHeight:function(){if(this.options.shouldToggleNavHeight){var $ul=this.data('ul-nav');var height=$ul.css('height');$ul.css('height',height==this.options.height?this.options.tallHeight:this.options.height);}},appendUI:function($elem){var $div=$('<div></div>')
.css('border','1px solid lightgray')
.css('padding','2px')
.append($('<h5></h5>')
.addClass('text-left')
...
.bind('click',function(e){$(this).next().collapse('toggle');}));var navHeight=this.options.height;var $ul=$('<ul></ul>')
.addClass('nav nav-list')
.css('height',navHeight)
.css('overflow','auto')
.attr('id','ul-nav');var $controls=$('<div></div>')
.addClass('btn-toolbar')
.addClass('text-right')
.append($('<div></div>')
.addClass('btn-group')
...
.attr('id','fileInput')
.css('display','none')
.bind('change',jQuery.proxy(this.handleFileSelect,this))));var $container=$('<div></div>')
.append($ul);$div.append($container);$elem.append($div);if(this.options.controls){if(this.options.externalControls){$div.append($controls);}
else{$container.append($controls);}}
this._rewireIds($div,this);$.each(this.options.controlButtons,$.proxy(function(idx,val){this.data('control-buttons').append(this[val]());},this));this._rewireIds($div,this);this.listDirectory(this.options.root,$ul);return this;},addDirectoryButton:function(){return this.data('addDirectoryButton')!=undefined?this.data('addDirectoryButton'):$('<a></a>')
.attr('id','addDirectoryButton')
.addClass('btn btn-mini')
.append($('<i></i>').addClass('icon-plus'))
.attr('title','Add new directory')
...
.attr('title','View selected file')
.tooltip()
.bind('click',$.proxy(function(e){e.preventDefault();if(!this.viewButton().hasClass('disabled')){this.openFile(this.data('activeFile'));}},this))},deleteButton:function(){return this.data('deleteButton')!=undefined?this.data('deleteButton'):$('<a></a>')
.attr('id','deleteButton')
.addClass('btn btn-mini disabled')
.append($('<i></i>').addClass('icon-remove'))
.attr('title','Delete selected item')
.tooltip()
.bind('click',$.proxy(function(e){e.preventDefault();if(!this.deleteButton().hasClass('disabled')){this.deleteFile();}},this))},uploadButton:function(){return this.data('uploadButton')!=undefined?this.data('uploadButton'):$('<a></a>')
.attr('id','uploadButton')
.addClass('btn btn-mini')
.append($('<i></i>').addClass('icon-arrow-up'))
.attr('title','Upload file')
.tooltip()
.bind('click',$.proxy(function(e){this.data('fileInput').trigger('click');},this))},addDirectory:function(){var parentDir=kbasePrompt(this.data('activeDirectory')||this.options.root;var that=this;var displayDir=parentDir.replace(this.options.root,'/');var $addDirectoryModal=$('<div></div>'), {title:'Create directory',body:$('<p></p>')
.append('Create directory ')
.append($('<span></span>')
.css('font-weight','bold')
.text(displayDir))
...
.attr('type','text')
.attr('name','dir_name')
.attr('size','20')),controls:['cancelButton',{name:'Create directory',type:'primary',callback:function(e,$prompt){$prompt.closePrompt();that.client.make_directory_async(that.sessionId,parentDir,$addDirectoryModal.dialogModal().find('input').val(),function(res){that.refreshDirectory(parentDir)},function(){});}}]});$addDirectoryModal.openPrompt();},openFile:function(file){var win=window.open();win.document.open();this.client.get_file_async(this.sessionId(),file,'/',$.proxy(function(res){try{var obj=JSON.parse(res);res=JSON.stringify(obj,undefined,2);}
catch(e){this.dbg("FAILURE");this.dbg(e);}
win.document.write($('<div></div>').append($('<div></div>')
.css('white-space','pre')
.append(res))
.html());win.document.close();},this),function(err){this.dbg("FILE FAILURE");this.dbg(err)});},deleteFile:function(){var file=this.data('activeFile');var deleteMethod='remove_files_async';if(file==undefined){file=this.data('activeDirectory');if(file==undefined){return;}
file=file.replace(/\/+$/,'');deleteMethod='remove_directory_async';}
var matches=file.match(/(.+)\/[^/]+$/);var active_dir='/';if(matches!=undefined&&matches.length>1){active_dir=matches[1];}
var that=kbaseDeletePrompt(this;var promptFile=file.replace(this.options.root,'');var $deleteModal=$('<div></div>'), {name:promptFile,callback:function(e,$prompt){$prompt.closePrompt();that.client[deleteMethod](that.sessionId,'/',file,function(res){that.refreshDirectory(active_dir)},function(){});}});$deleteModal.openPrompt();},handleFileSelect:function(evt){evt.stopPropagation();evt.preventDefault();var files=evt.target.files||evt.originalEvent.dataTransfer.files||evt.dataTransfer.files;$.each(files,jQuery.proxy(function(idx,file){var reader=new FileReader();var upload_dir='/';if(this.data('activeDirectory')){upload_dir=this.data('activeDirectory');}
var $processElem;if(this.options.processList){$processElem=this.options.processList.addProcess('Uploading '+file.name);}
reader.onload=jQuery.proxy(function(e){this.client.put_file_async(this.sessionId(),file.name,e.target.result,upload_dir,jQuery.proxy(function(res){if(this.options.processList){this.options.processList.removeProcess($processElem);}
...
this.dbg(res);},this));},this);reader.readAsText(file);},this));this.data('fileInput').val('');}});}(jQuery));(function($,undefined){return KBWidget("kbaseIrisGrammar",'kbaseWidget',{version:"1.0.0",options:{defaultGrammarURL:'http://www.prototypesite.net/iris-dev/grammar.json',},init:function(options){this._super(options);if(this.options.$loginbox!=undefined){this.$loginbox=this.options.$loginbox;}
...
this.$loginbox=kbaseLogin($("<div></div>"), {style:'text',login_callback:jQuery.proxy(function(args){if(args.success){this.out_line();this.client.start_session_async(args.user_id,jQuery.proxy(function(newsid){this.set_session(args.user_id);this.loadCommandHistory();this.out("Set session to "+args.user_id);this.scroll();},this),jQuery.proxy(function(err){this.out("<i>Error on session_start:<br>"+
err.message.replace("\n","<br>\n")+"</i>");},this));this.kbase_sessionid=kbaseIrisTutorial(args.kbase_sessionid;this.input_box.focus();this.refreshFileBrowser();}},this),logout_callback:jQuery.proxy(function(){this.sessionId=undefined;this.cwd='/';this.dbg("LOGOUT CALLBACK");this.refreshFileBrowser();this.terminal.empty();},this)});this.tutorial=$('<div></div>'));this.commandHistory=[];this.commandHistoryPosition=0;this.path='.';this.cwd="/";this.variables={};this.aliases={};this.appendUI($(this.$elem));var cookie;if(cookie=this.$loginbox.get_kbase_cookie()){var $commandDiv=$("<div></div>").css('white-space','pre');this.terminal.append($commandDiv);this.out_line();if(cookie.user_id){this.out_to_div($commandDiv,'Already logged in as '+cookie.name+"\n");this.set_session(cookie.user_id);this.loadCommandHistory();this.out_to_div($commandDiv,"Set session to "+cookie.user_id);}
...
.bind('click',function(e){var win=window.open();win.document.open();var output=$('<div></div>')
.append($('<div></div>')
.css('white-space','pre')
.css('font-family','monospace')
.append($(this).parent().parent().next().clone()));$.each(output.find('a'),function(idx,val){$(val).replaceWith($(val).html());});win.document.write(output.html());win.document.close();}))
.append($('<a></a>')
.addClass('btn btn-mini')
.append($('<i></i>')
.addClass('icon-remove'))
.attr('title','delete command from window')
.bind('click',function(e){$(this).parent().parent().next().remove();$(this).parent().parent().next().remove();$(this).parent().parent().remove();})))
.append($('<span></span>')
.addClass('command')
.text(">"+this.cwd+" "+text))
.mouseover(function(e){$(this).children().first().show();})
.mouseout(function(e){$(this).children().first().hide();});this.terminal.append($wrapperDiv);},out:function(text,scroll,html){this.out_to_div(this.terminal,text,scroll,html);},out_to_div:function($div,text,scroll,html){if(!html&&typeof text=='string'){text=text.replace(/</g,'&lt;');text=text.replace(/>/g,'&gt;');}
...
sid=args[0];this.client.start_session_async(sid,jQuery.proxy(function(newsid){this.set_session(sid);this.loadCommandHistory();this.out_to_div($commandDiv,"Set session to "+sid);},this),jQuery.proxy(function(err){this.out_to_div($commandDiv,"<i>Error on session_start:<br>"+
...
dir=args[0];this.client.change_directory_async(this.sessionId,this.cwd,dir,jQuery.proxy(function(path){this.cwd=path;},this),jQuery.proxy(function(err){var m=err.message.replace("/\n","<br>\n");this.out_to_div($commandDiv,"<i>Error received:<br>"+err.code+"<br>"+m+"</i>");},this));return;}
...
from=args[0];to=args[1];this.client.copy_async(this.sessionId,this.cwd,from,to,$.proxy(function(){this.refreshFileBrowser();},this),jQuery.proxy(function(err){var m=err.message.replace("\n","<br>\n");this.out_to_div($commandDiv,"<i>Error received:<br>"+err.code+"<br>"+m+"</i>");},this));return;}
if(m=command.match(/^mv\s*(.*)/)){var args=m[1].split(/\s+/)
if(args.length!=2){this.out_to_div($commandDiv,"Invalid mv syntax.");return;}
from=args[0];to=args[1];this.client.rename_file_async(this.sessionId,this.cwd,from,to,$.proxy(function(){this.refreshFileBrowser();},this),jQuery.proxy(function(err){var m=err.message.replace("\n","<br>\n");this.out_to_div($commandDiv,"<i>Error received:<br>"+err.code+"<br>"+m+"</i>");},this));return;}
if(m=command.match(/^mkdir\s*(.*)/)){var args=m[1].split(/\s+/)
if(args.length!=1){this.out_to_div($commandDiv,"Invalid mkdir syntax.");return;}
dir=args[0];this.client.make_directory_async(this.sessionId,this.cwd,dir,$.proxy(function(){this.refreshFileBrowser();},this),jQuery.proxy(function(err){var m=err.message.replace("\n","<br>\n");this.out_to_div($commandDiv,"<i>Error received:<br>"+err.code+"<br>"+m+"</i>");},this));return;}
if(m=command.match(/^rmdir\s*(.*)/)){var args=m[1].split(/\s+/)
if(args.length!=1){this.out_to_div($commandDiv,"Invalid rmdir syntax.");return;}
dir=args[0];this.client.remove_directory_async(this.sessionId,this.cwd,dir,$.proxy(function(){this.refreshFileBrowser();},this),jQuery.proxy(function(err){var m=err.message.replace("\n","<br>\n");this.out_to_div($commandDiv,"<i>Error received:<br>"+err.code+"<br>"+m+"</i>");},this));return;}
if(m=command.match(/^rm\s*(.*)/)){var args=m[1].split(/\s+/)
if(args.length!=1){this.out_to_div($commandDiv,"Invalid rm syntax.");return;}
file=args[0];this.client.remove_files_async(this.sessionId,this.cwd,file,$.proxy(function(){this.refreshFileBrowser();},this),jQuery.proxy(function(err){var m=err.message.replace("\n","<br>\n");this.out_to_div($commandDiv,"<i>Error received:<br>"+err.code+"<br>"+m+"</i>");},this));return;}
...
.attr('href','#')
.append(val)
.bind('click',$.proxy(function(e){e.preventDefault();e.stopPropagation();this.out_to_div($commandDiv,'Set tutorial to <i>'+val+'</i><br>',0,1);this.tutorial=kbaseIrisTutorial($('<div></div>'), {tutorial:val});this.input_box.focus();},this))
...
if(command=='commands'){this.client.valid_commands_async(jQuery.proxy(function(cmds){var $tbl=$('<table></table>');jQuery.each(cmds,function(idx,group){$tbl.append($('<tr></tr>')
.append($('<th></th>')
.attr('colspan',2)
...
this.client.list_files_async(this.sessionId,this.cwd,d,jQuery.proxy(function(filelist){var dirs=filelist[0];var files=filelist[1];var $tbl=$('<table></table>')
...
.html(val['mod_date']))
.append($('<td></td>')
.html(val['name'])));});jQuery.each(files,jQuery.proxy(function(idx,val){var url=this.options.invocationURL+"/download/"+val['full_path']+"?session_id="+this.sessionId;$tbl.append($('<tr></tr>')
.append($('<td></td>')
.text(val['size']))
.append($('<td></td>')
.html(val['mod_date']))
...
this.client.run_pipeline_async(this.sessionId,command,[],this.options.maxOutput,this.cwd,jQuery.proxy(function(runout){if(this.data('processList')){this.data('processList').removeProcess($pendingProcessElem);}
