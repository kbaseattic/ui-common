...
.attr('id','fileInput')
.css('display','none')
.bind('change',jQuery.proxy(this.handleFileSelect,this))));return $div;},appendUI:function($elem){var $container= new kbaseBox(this.fileBrowserContainer();var $box=$('<div></div>'), {title:this.options.name,canCollapse:true,content:$container,});$elem.append($box.$elem);if(this.options.controls){if(this.options.externalControls){$container.parent().parent().append(this.fileBrowserControls());}
...
.data('require','f')},addDirectory:function(){var parentDir= new kbasePrompt(this.data('activeDirectory')||this.options.root;var that=this;var displayDir=parentDir.replace(this.options.root,'/');var $addDirectoryModal=$('<div></div>'), {title:'Create directory',body:$('<p></p>')
...
.attr('title','Upload file')
.tooltip()
.bind('click',$.proxy(function(e){this.data('fileInput').trigger('click');},this))},addDirectory:function(){var parentDir= new kbasePrompt(this.data('activeDirectory')||this.options.root;var that=this;var displayDir=parentDir.replace(this.options.root,'/');var $addDirectoryModal=$('<div></div>'), {title:'Create directory',body:$('<p></p>')
.append('Create directory ')
.append($('<span></span>')
.css('font-weight','bold')
.text(displayDir))
...
.attr('href','#')
.append(val)
.bind('click',$.proxy(function(e){e.preventDefault();e.stopPropagation();this.out_to_div($commandDiv,'Set tutorial to <i>'+val+'</i><br>',0,1);this.tutorial= new kbaseIrisTutorial($('<div></div>'), {tutorial:val});this.input_box.focus();},this))
