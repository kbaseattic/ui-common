(function($,undefined){return KBWidget("kbaseIrisCommands",'kbaseAccordion',{version:"1.0.0",options:{link:function(evt){alert("clicked on "+$(evt.target).text());},englishCommands:0,fontSize:'90%',overflow:true,sectionHeight:'300px',},init:function(options){this._super(options);if(options.client){this.client=options.client;}
...
.on('click',$.proxy(function(e){e.preventDefault();e.stopPropagation();this.data('searchField').trigger('keyup');},this)))));var $box=kbaseBox($.jqElem('div'), {'title':'Command list','content':$.jqElem('div')
.append($.jqElem('div')
...
.append($form),});$elem.append($box.$elem);this._rewireIds($box.$elem,this);this._superMethod('appendUI',this.data('all-commands'),commands);this.data('accordion').css('margin-bottom','0px');},});}(jQuery));(function($,undefined){return KBWidget("kbaseIrisFileBrowser",'kbaseDataBrowser',{version:"1.0.0",_accessors:['invocationURL','client','addFileCallback','editFileCallback','singleFileSize','chunkSize','stalledUploads'],options:{stalledUploads:{},uploadDir:'.uploads',concurrentUploads:4,singleFileSize:15000000,chunkSize:5000000,title:'File Browser','root':'/',types:{file:{controls:[{icon:'icon-minus',callback:function(e,$fb){$fb.deleteFile($(this).data('id'),'file');},id:'removeButton',},{icon:'icon-download-alt',callback:function(e,$fb){$fb.openFile($(this).data('id'));},id:'viewButton',},{icon:'icon-pencil',callback:function(e,$fb){if($fb.editFileCallback()!=undefined){$fb.editFileCallback()($(this).data('id'),$fb);}},id:'editButton',condition:function(control,$fb){var size=this.$elem.data('data').size;if(size>$fb.singleFileSize()){return false;}
...
kbaseButtonControls($pc, {onMouseover:false,context:this,controls:[{'icon':'icon-plus',callback:function(e,$fb){$fb.addDirectory('/');},},{'icon':'icon-arrow-up',callback:function(e,$fb){$fb.data('active_directory',$(this).data('id'));$fb.data('fileInput').trigger('click');}},]});return $('<div></div>')
...
var that=kbaseDeletePrompt(kbasePrompt(this;var promptFile=file.replace(this.options.root,'');var $deleteModal=$('<div></div>'), {name:promptFile,callback:function(e,$prompt){$prompt.closePrompt();that.client()[deleteMethod](that.sessionId(),'/',file,function(res){that.refreshDirectory(active_dir)});}});$deleteModal.openPrompt();},addDirectory:function(parentDir){var that=this;var displayDir=parentDir.replace(this.options.root,'/');var $addDirectoryModal=$('<div></div>'), {title:'Create directory',body:$('<p></p>')
...
return;}
var $saveModal=kbasePrompt($.jqElem('div'), {title:'Save changes',body:'Save changes to <strong>'+this.file()+'</strong> before closing?',controls:[{name:'Close without save',callback:$.proxy(function(e,$prompt){$prompt.closePrompt();if(this.cancelSaveFileCallback){this.cancelSaveFileCallback()(this.file());}},this)},{name:'Close and save',type:'primary',callback:$.proxy(function(e,$prompt){$prompt.closePrompt();this.save();if(this.saveFileCallback){this.saveFileCallback()(this.file());}},this)}],});$saveModal.openPrompt();},});}(jQuery));(function($,undefined){return KBWidget("kbaseIrisGrammar",'kbaseWidget',{version:"1.0.0",options:{defaultGrammarURL:'http://www.prototypesite.net/iris-dev/grammar.json',},init:function(options){this._super(options);if(this.options.$loginbox!=undefined){this.$loginbox=this.options.$loginbox;}
...
this.processList()[pid]=kbaseBox(undefined;return pid;},appendUI:function($elem){var $box=$elem, {'title':'Running processes','content':$('<ul></ul>')
.addClass('unstyled')
...
this.tutorial=kbaseIrisTutorial($('<div></div>'));this.commandHistory=[];this.commandHistoryPosition=0;this.path='.';this.cwd="/";this.variables={};this.aliases={};this.appendUI($(this.$elem));this.fileBrowsers=[];if(this.options.fileBrowser){this.addFileBrowser(this.options.fileBrowser);}
kbaseIrisFileBrowser(else if(this.options.autocreateFileBrowser){this.addFileBrowser($('<div></div>'), {client:this.client(),externalControls:false,}))};return this;},loggedInCallback:function(e,args){if(args.success){this.client().start_session(args.user_id,$.proxy(function(newsid){this.loadCommandHistory();if(args.token){this.out("Authenticated as "+args.name);}
...
kbaseButtonControls(.mouseout(function(e){$(this).children().first().hide();});$wrapperDiv, {controls:[{icon:'icon-eye-open',callback:function(e){var win=window.open();win.document.open();var output=$('<div></div>')
.append($('<div></div>')
.css('white-space','pre')
.css('font-family','monospace')
