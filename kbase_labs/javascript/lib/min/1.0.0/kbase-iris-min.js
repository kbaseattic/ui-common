...
.on('click',$.proxy(function(e){e.preventDefault();e.stopPropagation();this.data('searchField').trigger('keyup');},this)))));var $box= new kbaseBox($.jqElem('div'), {'title':'Command list','content':$.jqElem('div')
.append($.jqElem('div')
...
.always($.proxy(function(res){this.checkStalledUploads();},this));}},loggedOutCallback:function(e){this.data('ul-nav').empty();},prepareRootContent:function(){var $ul=this._super();$ul.css('height',(parseInt(this.options.height)-25)+'px');var $pc=$('<div></div>').css('margin-top','2px')
 new kbaseButtonControls($pc, {onMouseover:false,context:this,controls:[{'icon':'icon-plus',callback:function(e,$fb){$fb.addDirectory('/');},},{'icon':'icon-arrow-up',callback:function(e,$fb){$fb.data('active_directory',$(this).data('id'));$fb.data('fileInput').trigger('click');}},]});return $('<div></div>')
...
this.processList()[pid]= new kbaseBox(undefined;return pid;},appendUI:function($elem){var $box=$elem, {'title':'Running processes','content':$('<ul></ul>')
.addClass('unstyled')
...
this.tutorial= new kbaseIrisTutorial($('<div></div>'));this.commandHistory=[];this.commandHistoryPosition=0;this.path='.';this.cwd="/";this.variables={};this.aliases={};this.appendUI($(this.$elem));this.fileBrowsers=[];if(this.options.fileBrowser){this.addFileBrowser(this.options.fileBrowser);}
 new kbaseIrisFileBrowser(else if(this.options.autocreateFileBrowser){this.addFileBrowser($('<div></div>'), {client:this.client(),externalControls:false,}))};return this;},loggedInCallback:function(e,args){if(args.success){this.client().start_session(args.user_id,$.proxy(function(newsid){this.loadCommandHistory();if(args.token){this.out("Authenticated as "+args.name);}
...
.mouseover(function(e){$(this).children().first().show();})
 new kbaseButtonControls(.mouseout(function(e){$(this).children().first().hide();});$wrapperDiv, {controls:[{icon:'icon-eye-open',callback:function(e){var win=window.open();win.document.open();var output=$('<div></div>')
.append($('<div></div>')
.css('white-space','pre')
.css('font-family','monospace')
