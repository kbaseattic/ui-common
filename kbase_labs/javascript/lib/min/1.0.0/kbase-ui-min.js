...
.css('font-size','70%')
.css('color','gray')
 new kbaseButtonControls(.append(this.options.postcontent));this._rewireIds($div,this);if(this.options.controls){this.data('banner-text'), {onMouseover:false,controls:this.options.controls})}
...
else{var $errorModal= new kbasePrompt($('<div></div>'), {title:'Login failed',body:$('<div></div>')
.attr('class','alert alert-error')
.append($('<div></div>')
.append($('<div></div>')
.addClass('pull-left')
...
if(value!=undefined){$tr.append($td);}},this));}
return $tr;},deletePrompt:function(row){var $deleteModal= new kbaseDeletePrompt($('<div></div>'), {name:row,callback:this.deleteRowCallback(row),});$deleteModal.openPrompt();},deleteRowCallback:function(row){},shouldDeleteRow:function(row){return 1;},});}(jQuery));(function($,undefined){return KBWidget("kbaseTabs",'kbaseWidget',{version:"1.0.0",_accessors:['tabsHeight'],options:{tabPosition:'top',canDelete:false,borderColor:'lightgray',},init:function(options){this._super(options);this.data('tabs',{});this.data('nav',{});this.appendUI($(this.$elem));return this;},appendUI:function($elem,tabs){if(tabs==undefined){tabs=this.options.tabs;}
var $block=$('<div></div>')
...
var controls=val.controls;if(controls==undefined&&val.type!=undefined){controls=this.options.types[val.type].controls;}
 new kbaseButtonControls(if(controls){$li, {controls:controls,id:val.id,context:this});}},this));this._rewireIds($target,this);return $target;},prepareRootContent:function(){return $('<ul></ul>')
.addClass('nav nav-list')
.css('height',this.options.height)
.css('overflow','auto')
.attr('id','ul-nav');},appendUI:function($elem){var $root= new kbaseBox(this.prepareRootContent();this._rewireIds($root,this);this.appendContent(this.options.content,this.data('ul-nav'));$elem, {title:this.options.title,canCollapse:this.options.canCollapse,content:$root,postcontent:this.options.postcontent});return $elem;},});}(jQuery));