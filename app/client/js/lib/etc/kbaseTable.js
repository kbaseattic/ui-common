...
eap: 7/1/15 - refactoring to be be an AMD module, requirejs style; also, ditch the global registration business; can easily add something like this on top of the base table object.
    ...
    function ($) {
        ...
        var table = {
            ...
            create: function (params) {
                ...
                jQuery.extend(true, instance.settings, renderer.defaults, params);
            ...
            render: function (index) {
                            ...
                            switch (sorttype[sortcol]) {
                                case "number":
                                    if ((typeof (a[header[sortcol]].replace) != 'function') || (typeof (b[header[sortcol]].replace) != 'function')) {
                                        if (a[header[sortcol]] == b[header[sortcol]])
                                            return 0;
                                        if (a[header[sortcol]] < b[header[sortcol]])
                                            return -1;
                                    } else {
                ...
                if (offset > 0) {
                    ...
                    first.index = index;
                    first.onclick = typeof renderer.settings.navigation_callback == "function" ? function () {
                        var index = this.index;
                        var renderer = rendererTable[index];
                    ...
                    prev.index = index;
                    prev.onclick = typeof renderer.settings.navigation_callback == "function" ? function () {
                        var index = this.index;
                        var renderer = rendererTable[index];
                ...
                if (offset + rows < (renderer.settings.numrows || tdata.length)) {
                    ...
                    last.index = index;
                    last.onclick = typeof renderer.settings.navigation_callback == "function" ? function () {
                        var index = this.index;
                        var renderer = rendererTable[index];
                    ...
                    next.index = index;
                    next.onclick = typeof renderer.settings.navigation_callback == "function" ? function () {
                        var index = this.index;
                        var renderer = rendererTable[index];
